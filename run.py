import requests # for high level API calls, as recommended in Python documentation
...
def calculate_distance(user_lat, user_lon, location_lat, location_lon):
    ...
    This function initially used Pythagoras' Theorem, but later replaced it with
...
def calculate_distance_geopy(user_lat, user_lon, location_lat, location_lon):
    ...
    This function is only used for debugging/testing purposes, to ensure accuracy
    of the 'calculate_distance' function.
    """
...
def get_users_near_london(users, london_lat, london_lon):
    ...
    """
    users_near_london = get_london_users_json() # initial list is the starting point
    for u in users: # if a user is within 50 miles of London...
        if within_50_miles(
            ...
            london_lat,
            london_lon):
            new_record = True # sets a flag to check against to see if already present
            for l in users_near_london:
                if u == l: # If the record is already present in the list...
                    new_record = False # mark the flag
                    break # stop looping.
            if new_record:
                users_near_london.append(u) # if not, it must be new, therefore append it

    return users_near_london


def run():
    london_lat, london_lon = 51.509865, -0.118092 # coordinates from www.latlong.net
    users = get_users_json() # retrieves all users
    london_users = get_users_near_london( # then calculates which should be returned
        users,
        london_lat,
        london_lon)
